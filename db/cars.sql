...
group by crs.name, cb.name, ce.name, ct.name
...
select cb.name from car_bodies as cb left join cars as crs on crs.body_id = cb.id where crs.id is null;

select ce.name from car_engines as ce left join cars as crs on crs.engine_id = ce.id where crs.id is null;

select ct.name from car_transmissions as ct left join cars as crs on crs.transmission_id = ct.id where crs.id is null;